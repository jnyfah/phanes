...
- **Growth without freeing.** When it needs to grow, the old buffer is retained
  (not freed) to avoid use-after-free against in-flight thieves. This is a
  deliberate, documented trade-off — proper lock-free reclamation (hazard pointers
  / epoch-based) is future work.

> Gotcha worth knowing: because `LockFreeDeque` is 64-byte over-aligned, it must
> not be a *local variable inside a coroutine frame* — some compilers don't honor
> over-alignment for coroutine-frame locals, which causes misaligned atomics and
> intermittent crashes. The duplicate scanner heap-allocates it (`make_unique`) for
> exactly this reason.
